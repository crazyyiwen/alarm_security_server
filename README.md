...
1. 1 If the system not armed or disarmed, all users can issue an order without password.

1. 2 If the system armed, all users must issue an order with password, otherwise, system will deny.

1. 3 “Arm system“/”Disarm System”/”Door operation(open door)”, please use cmd like “I am …, please…“, this is to mock the voice to differentiate the Admin user or client user(LLM will parse the exact value from message).

1. 4 For now, I only setup one admin user, normal user can not remove admin user.

1. 5 Now, any valid user can add new user, should provide password, start_date and end_date are optional.

1. 6 If user input the password fails 3 times, system will block the user.

1. 7 if one user is locked due to multiple ties failure, another user can help to open the door.
1. 8 LLM will check if system armed, username, password, expiration time range fetching.
1. 9 LLM will check how many times password input failure, if multiple times failed, user will be locked.
